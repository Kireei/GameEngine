...
public class Loader {
	...
	public RawModel loadToVAO(float[] positions, float[] textureCoords, float[] normals, float[] colors, int[] indices){
		int vaoID = createVAO();
		bindIndicesBuffer(indices);
		storeDataInAttributeList(0, 3, positions);
		storeDataInAttributeList(1, 2, textureCoords);
	...
	public RawModel loadToVAO(float[] positions, float[] textureCoords, float[] normals, float[] colors, float[] lightingFactors, int[] indices){
		int vaoID = createVAO();
		bindIndicesBuffer(indices);
		storeDataInAttributeList(0, 3, positions);
		storeDataInAttributeList(1, 2, textureCoords);
		storeDataInAttributeList(2, 3, normals);
		storeDataInAttributeList(3, 3, colors);
		storeDataInAttributeList(4, 3, lightingFactors);
		unbindVAO();
		return new RawModel(vaoID, indices.length);
	}
